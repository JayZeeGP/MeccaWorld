...
tryAgain:
	...
	protected final void mNL(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		{
		boolean synPredMatched2513 = false;
		if (((LA(1)=='\r') && (LA(2)=='\n'))) {
			int _m2513 = mark();
			synPredMatched2513 = true;
			inputState.guessing++;
			try {
				{
				match("\r\n");
				...
				synPredMatched2513 = false;
			}
			rewind(_m2513);
inputState.guessing--;
		}
		if ( synPredMatched2513 ) {
			match("\r\n");
		}
		else if ((LA(1)=='\n')) {
			match('\n');
		...
		if ((LA(1)=='e')) {
			...
			do {
				if (((LA(1) >= '0' && LA(1) <= '9'))) {
					...
					if ( _cnt2551>=1 ) { break _loop2551; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
				}
